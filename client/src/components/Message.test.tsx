...
describe('Message Component', () => {
  ...
  describe('User Messages', () => {
    it('renders user message with correct content', () => {
      render(<Message role="user" content="Hello, assistant!" timestamp={mockTimestamp} />);
      expect(screen.getByText('Hello, assistant!')).toBeInTheDocument();
    });

    it('applies correct styling for user messages', () => {
      const { container } = render(
        <Message role="user" content="User message" timestamp={mockTimestamp} />
      );
  ...
  describe('Assistant Messages', () => {
    it('renders assistant message with correct content', () => {
      render(<Message role="assistant" content="How can I help you?" timestamp={mockTimestamp} />);
      expect(screen.getByText('How can I help you?')).toBeInTheDocument();
    });

    it('applies correct styling for assistant messages', () => {
      const { container } = render(
        <Message role="assistant" content="Assistant message" timestamp={mockTimestamp} />
      );
  ...
  describe('Timestamp Display', () => {
    it('displays formatted timestamp', () => {
      render(<Message role="user" content="Test message" timestamp={mockTimestamp} />);
      expect(screen.getByText('14:30:45')).toBeInTheDocument();
    });

    it('formats single-digit hours with leading zero', () => {
      const earlyTimestamp = new Date('2025-11-03T09:05:03');
      render(<Message role="user" content="Test message" timestamp={earlyTimestamp} />);
  ...
  describe('Edge Cases', () => {
    it('handles long messages without breaking layout', () => {
      const longMessage =
        'This is a very long message that should wrap properly without breaking the layout or extending beyond the maximum width constraints set by Tailwind CSS classes.';
      const { container } = render(
        <Message role="user" content={longMessage} timestamp={mockTimestamp} />
      );
    ...
    it('handles empty content', () => {
      render(<Message role="user" content="" timestamp={mockTimestamp} />);
    ...
    it('handles special characters in content', () => {
      ...
      render(<Message role="assistant" content={specialChars} timestamp={mockTimestamp} />);
      expect(screen.getByText(specialChars)).toBeInTheDocument();
    });

    it('handles multiline content', () => {
      const multilineContent = 'Line 1\nLine 2\nLine 3';
      render(<Message role="user" content={multilineContent} timestamp={mockTimestamp} />);
  ...
  describe('Responsive Design', () => {
    it('applies responsive max-width classes', () => {
      const { container } = render(
        <Message role="user" content="Test message" timestamp={mockTimestamp} />
      );
